...
const MultiSelectCate: React.FC<DropdownProps> = ({ id, optionsDefault = [], selectedDefault = [], onSelectionChange }) => {
    ...
    useEffect(() => {
        ...
        const loadOptions = () => {
            ...
            if (select) {
                ...
                const newSelected: number[] = []; // Biến tạm để giữ các giá trị đã chọn

                for (let i = 0; i < select.options.length; i++) {
                    const isSelected = selectedDefault.includes(select.options[i].innerText);
                    newOptions.push({
                ...
                setSelected(newSelected); // Cập nhật state một lần với tất cả các giá trị đã chọn
            }
        };
        loadOptions();
    }, [id, optionsDefault, selectedDefault]);
    const open = () => {
        setShow(true);
    };
    ...
    useEffect(() => {
        onSelectionChange(selected.map((option) => options[option].text));
    }, [selected]);
        ...
        <div className="relative z-50">
            ...
            <div>
                <select className="hidden" id={id} >
                    {optionsDefault && optionsDefault.map((option, index) => (
...
export default MultiSelectCate;
